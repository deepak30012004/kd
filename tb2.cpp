...
trainList.insert(12461, "Jodhpur - Sabarmati Vande Bharat Express", "VB", "NWR", "JU", "06:05", "SBIB", "12:05", "6h 0m", 5, "SMTWFS", "CC Ex", "449 km", "75 km/hr");
    ...
    std::string departingStation, reachingStation;
    std::cout << "Enter the departing station: ";
    std::cin >> departingStation;
    std::cout << "Enter the reaching station: ";
    std::cin >> reachingStation;

    std::vector<int> matchingRoutes;
    for (const auto &entry : trainRoutes)
    {
        const std::vector<std::string> &routeStations = entry.second;
        if (std::find(routeStations.begin(), routeStations.end(), departingStation) != routeStations.end() &&
            std::find(routeStations.begin(), routeStations.end(), reachingStation) != routeStations.end())
        {
            matchingRoutes.push_back(entry.first);
        }
    }

    if (matchingRoutes.empty())
    {
        std::cout << "No train routes found between the specified stations." << std::endl;
    }
    else
    {
        std::cout << "Train numbers operating between " << departingStation << " and " << reachingStation << " are: " << std::endl;
        for (const auto &route : matchingRoutes)
        {
            std::cout << route << std::endl;
             trainList.displayTrainInfo(route);
